...
- **`openshift_sscsi_vault.syncVaultCsiTlsCaConfigMapYaml`** — ConfigMap manifest when sync is enabled, **`createConfigMap`** is true (default), and PEM is non-empty.
...
Argo CD (and plain **`helm template`**) runs **client-side**: **`helm lookup()`** does not see the cluster API. For GitOps, set **`ocpSecretsStoreCsiVault.caProvider.syncProviderCaConfigMap.pemLiteral`** to the full PEM bundle (for example via SOPS-encrypted values or Argo Helm parameters), or set **`createConfigMap: false`** and install the ConfigMap plus Vault CSI volume mounts yourself. Use **`useLookup: true`** only when rendering with a live API (for example **`helm template --dry-run=server`** or **`helm install`**).
...
| ocpSecretsStoreCsiVault.caProvider | object | `{"clientCluster":{"defaultVaultCACertPath":"/etc/pki/vault-ca/hub-kube-root-ca.crt","key":"hub-kube-root-ca.crt","name":"hub-ca","namespace":"external-secrets","type":"Secret"},"enabled":true,"hostCluster":{"defaultVaultCACertPath":"/etc/pki/vault-ca/kube-root-ca.crt","key":"ca.crt","name":"kube-root-ca.crt","namespace":"external-secrets","type":"ConfigMap"},"syncProviderCaConfigMap":{"configMapName":"openshift-sscsi-vault-vault-tls-ca","createConfigMap":true,"enabled":false,"ingressRouterCa":{"key":"ca-bundle.crt","name":"router-ca","namespace":"openshift-ingress"},"keyInConfigMap":"vault-tls-ca.pem","mountDir":"/etc/pki/vault-ca","pemLiteral":"","preset":"auto","targetNamespace":"vault","useLookup":false}}` | Mirrors openshift-external-secrets-chart `ocpExternalSecrets.caProvider`: when enabled and `tls.vaultCACertPath` is empty, `vaultCACertPath` defaults per hub vs spoke (same branching as ESO). CSI reads a PEM path on the Vault CSI provider pod; `defaultVaultCACertPath` should match where you mount the same trust material as the type/name/key/namespace objects below. Nested: `syncProviderCaConfigMap`, `hostCluster`, `clientCluster`. |
...
| ocpSecretsStoreCsiVault.caProvider.syncProviderCaConfigMap | object | `{"configMapName":"openshift-sscsi-vault-vault-tls-ca","createConfigMap":true,"enabled":false,"ingressRouterCa":{"key":"ca-bundle.crt","name":"router-ca","namespace":"openshift-ingress"},"keyInConfigMap":"vault-tls-ca.pem","mountDir":"/etc/pki/vault-ca","pemLiteral":"","preset":"auto","targetNamespace":"vault","useLookup":false}` | TLS CA sync for Vault HTTPS: `pemLiteral` / `createConfigMap` for GitOps; optional `lookup` when `useLookup` is true. See chart README (Argo CD section). |
| ocpSecretsStoreCsiVault.caProvider.syncProviderCaConfigMap.configMapName | string | `"openshift-sscsi-vault-vault-tls-ca"` | ConfigMap name to create or reference; mount this name on the Vault CSI provider DaemonSet. |
| ocpSecretsStoreCsiVault.caProvider.syncProviderCaConfigMap.createConfigMap | bool | `true` | When false, this chart does not create the ConfigMap; create and mount it elsewhere. SPC still receives `vaultCACertPath` at `mountDir`/`keyInConfigMap` when sync is enabled. |
| ocpSecretsStoreCsiVault.caProvider.syncProviderCaConfigMap.enabled | bool | `false` | When true, emit a ConfigMap (unless `createConfigMap` is false) when PEM is available, and set SPC `vaultCACertPath` for the mount path under `mountDir`. |
...
| ocpSecretsStoreCsiVault.caProvider.syncProviderCaConfigMap.keyInConfigMap | string | `"vault-tls-ca.pem"` | Key within the ConfigMap whose value is the PEM file (single file item in the volume). |
...
| ocpSecretsStoreCsiVault.caProvider.syncProviderCaConfigMap.pemLiteral | string | `""` | Inline PEM (full bundle). GitOps-friendly: SOPS-encrypted values, Argo CD helm parameters, or a pattern override file. When set, used instead of cluster lookup. |
...
| ocpSecretsStoreCsiVault.caProvider.syncProviderCaConfigMap.targetNamespace | string | `"vault"` | Namespace for the synced ConfigMap (pattern: `vault` where the Vault CSI provider runs). The release ServiceAccount must be allowed to create ConfigMaps here unless you manage the CM out-of-band. |
