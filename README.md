...
Argo CD (and plain **`helm template`**) runs **client-side**: **`helm lookup()`** does not see the cluster API. By default **`syncProviderCaConfigMap.enabled: true`**, **`createConfigMap: true`**, and **`injectTrustedCabundle: true`**: the chart renders a **`ConfigMap`** labeled **`config.openshift.io/inject-trusted-cabundle: "true"`** (empty **`data`** until the Cluster Network Operator fills **`trustedCabundleDataKey`**, default **`ca-bundle.crt`**). The ConfigMap includes **`argocd.argoproj.io/ignore-differences`** with JSON pointer **`/data/<trustedCabundleDataKey>`** (disable with **`argocdIgnoreInjectedTrustedCabundleData: false`**); if your Argo CD build does not honor per-resource ignore-differences yet, add the same path under the **Application** **`spec.ignoreDifferences`** and use sync option **`RespectIgnoreDifferences=true`**. Mount only that key on the Vault CSI DaemonSet with a **`projected`** volume (**`configMap.items`** + **`optional: true`**) so the pod tolerates the key appearing after CNO injection (see aap-starter-kit **`overrides/values-vault-csi-tls-ca.yaml`**). The SecretProviderClass sets **`vaultCACertPath`** to **`mountDir`/`trustedCabundleDataKey`**. For the previous **rhvp.cluster_utils** flow (Ansible creates **`vault-tls-ca.pem`**), set **`injectTrustedCabundle: false`**, **`createConfigMap: false`**, and avoid duplicate applies for the same **`configMapName`**. Alternatively set **`pemLiteral`** or **`createConfigMap: true`** with **`useLookup: true`** for cluster-side Helm only.
...
* v0.0.16: CNO inject **`ConfigMap`** defaults **`argocd.argoproj.io/ignore-differences`** to **`/data/<trustedCabundleDataKey>`** (toggle **`argocdIgnoreInjectedTrustedCabundleData`**); optional **`configMapAnnotations`**. Patterns should mount the bundle via **`projected`** **`items`** + **`optional: true`** on the Vault CSI DaemonSet (see aap-starter-kit **`values-vault-csi-tls-ca.yaml`**).
...
| ocpSecretsStoreCsiVault.caProvider.syncProviderCaConfigMap.argocdIgnoreInjectedTrustedCabundleData | bool | `true` | When `injectTrustedCabundle` and this chart renders the TLS ConfigMap: set `argocd.argoproj.io/ignore-differences` to a JSON pointer `/data/<trustedCabundleDataKey>` so Argo CD can skip drift on CNO-managed content (requires Argo CD / OpenShift GitOps that honors per-resource ignore-differences, or duplicate the rule under the Application `spec.ignoreDifferences`; see README). |
